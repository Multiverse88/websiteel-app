...
export const api = {
  ...
  getWaLeadsStats: async (groupBy: 'day' | 'week' | 'month' | 'number' | 'source' | 'service', filters: { status?: string; numberId?: string; domain?: string; source?: string; from?: string; to?: string; excludeBot?: string } = {}) => {
  ...
  exportWa: async (filters: { format?: 'xlsx' | 'csv'; type?: 'leads' | 'numbers' | 'all'; ai?: string; status?: string; numberId?: string; domain?: string; source?: string; product?: string; search?: string; from?: string; to?: string; excludeBot?: string } = {}) => {
    const qs = new URLSearchParams(Object.entries(filters).filter(([, v]) => v) as [string, string][]).toString()
  ...
  // Analytics deep-dive & detailed multi-dimension charts
  getAnalyticsDetail: async (params: { domain?: string; from?: string; to?: string; groupBy?: 'day' | 'week' | 'month'; excludeBot?: boolean } = {}) => {
    const query = new URLSearchParams();
    if (params.domain && params.domain !== 'all') query.set('domain', params.domain);
    if (params.from) query.set('from', params.from);
    if (params.to) query.set('to', params.to);
    ...
    if (params.excludeBot !== undefined) query.set('excludeBot', params.excludeBot ? '1' : '0');
    const queryString = query.toString() ? `?${query.toString()}` : '';
    const res = await authenticatedFetch(`${API_BASE_URL}/analytics/detail${queryString}`);
