...
export default function Dashboard() {
  const { user } = useAuth()
  const [stats, setStats] = useState({ articles: 0, contacts: 0, newsletter: 0, landingPages: 0, blastContacts: 0 })
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    async function load() {
      try {
        const [articles, contacts, newsletter, landingPages, blastContacts] = await Promise.allSettled([
          api.getArticles(),
          api.getContacts(),
          api.getNewsletter(),
          api.getLandingPages(),
          api.getBlastContacts(),
        ])
        setStats({
          ...
          blastContacts: blastContacts.status === 'fulfilled' ? (Array.isArray(blastContacts.value) ? blastContacts.value.length : 0) : 0,
        })
      } catch (e) {
        console.error(e)
      } finally {
  ...
  return (
    <div className="page">
      ...
      <div className="stat-grid">
        ...
        <StatCard title="Blast Contacts" value={loading ? '...' : stats.blastContacts} icon="📧" />
      </div>
      <div className="quick-actions">
        ...
        <div className="quick-actions-grid">
          ...
          <a href="#/email-blast" className="btn btn--outline">Email Blast</a>
        </div>
      </div>
    </div>
  )
