...
router.post("/process-queue", requireCronSecret, async (req, res) => {
  try {
    ...
    await Promise.all(pendingRecipients.map(async (recipient: any) => {
      ...
      const pixelUrl = `https://easylegal.biz.id/api/track-open/${recipient.id}`;
      ...
      finalHtml = finalHtml.replace(/href="([^"]+)"/g, (match: string, url: string) => {
        ...
        return `href="https://easylegal.biz.id/api/track-click/${recipient.id}?url=${encoded}"`;
      });
