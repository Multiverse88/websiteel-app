...
async function fetchArticleFromApi(slug: string, site: string) {
  // site=easylegal.id also picks up legacy biz.id-seeded articles (backend
  // buildSiteFilter()) so old URLs keep resolving; new posts targeted at
  // this domain specifically use the dashboard's Site selector.
...
function cleanArticleUrl(url: string, articleTitle: string): string {
  ...
  if (url.includes("/lp-produk-koperasi") || url.includes("easylegal.id/lp-produk-koperasi") || url.includes("easylegal.biz.id/lp-produk-koperasi") || url.includes("easylegal.co.id/lp-produk-koperasi")) {
